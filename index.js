    ...
    for (const img of images) {
        ...
        <div class="mes_media_container mes_img_container chat-image-queued" data-index="${Date.now()}">
            <div class="chat-image-frame" style="max-width:100%;background:rgba(${bgRgb},${bgOpacity});">
                <img class="mes_img" src="${imgUrl}" width="${imgW}" height="${imgH}" style="max-width:100%;max-height:100%;width:${imgW}px;height:${imgH}px;object-fit:contain;" alt="${escapeHtml(img.name || '聊天图片')}" title="${escapeHtml(img.name || '聊天图片')}" onerror="chatImagesCleanupStaleImage(this)">
            </div>
        </div>`;
